...
public class SkinLoader : MonoBehaviour//, IWantsAds
{
	...
	private void Initialise()
	{
		...
		if (unlockWeapons)
		{
			if (ShopStateController.CurrentState.AreAllWagonsUnlocked()) return;
		...
		{
			if (ShopStateController.CurrentState.AreAllCharacterSkinsUnlocked()) return;
	...
	private void MoneyPanelButtonBehaviour()
	{
		...
		DOVirtual.DelayedCall(2f, () =>
		{
			...
			if (unlockWeapons ?
				ShopStateController.CurrentState.AreAllWagonsUnlocked() :
				ShopStateController.CurrentState.AreAllCharacterSkinsUnlocked()
	...
	private static void FindNewLoaderWeapon(int currentIndex)
	{
		...
		{
			if (ShopStateController.CurrentState.GetState().wagonStates[(WeaponType) i] != ShopItemState.Locked)
				continue;
	...
	private static void FindNewLoaderArmsSkin(int currentIndex)
	{
		...
		{
			if (ShopStateController.CurrentState.GetState().characterStates[(ArmsType) i] != ShopItemState.Locked)
				continue;
